...
- `VIRTUALENVWRAPPER_VIRTUALENV_ARGS='--system-site-packages'` must be set in the user's shell environment. The baseenv holds explicitly managed packages; transitive deps and system-level packages (GDAL, PyQt6, etc.) are made available through `--system-site-packages` on user virtualenvs. The CI test venv is created with this flag to mirror real usage.
